...
static void InitOpenGL(GraphicsState *state) {
  ...
  int success = graphics_get_display_size(0 /* LCD */, &state->screen_width, &state->screen_height);
  assert(success >= 0);
...
static void LoadWormholeTexture(GraphicsState* state) {
  ...
  glBindTexture(GL_TEXTURE_2D, state->tex);
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, state->screen_width, state->screen_height, 0, GL_RGB, GL_UNSIGNED_BYTE, buf);
    free(buf);
  }
